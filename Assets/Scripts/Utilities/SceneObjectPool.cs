...
using Reflectrix.Utilities.ObjectPool;
using UnityEngine;
...
namespace Reflectrix.Utilities
{
    ...
    {
        private const int PoolsMaxSize = 20;
        ...
        private bool SpawnInternal<T>(PoolObject prefab, out T obj, bool spawnDeactivated = false) where T : PoolObject
        {
            ...
            {
                pool = new ObjectPool<PoolObject>(PoolsMaxSize);
            ...
            if (gotObject is T casted)
            {
                obj = casted;
            }
            else
            {
                throw new InvalidCastException($"Pooled object is not of type {typeof(T)}");
            }
...
# if UNITY_EDITOR
        ...
        private void DespawnInternal(PoolObject obj)
        {
            ...
            if (!gameObjectPrefabs.TryGetValue(obj.GetInstanceID(), out var prefabId))
            {
                Debug.LogWarning($"[SceneObjectPool] Trying to despawn unmanaged object: {obj.name}");
                return;
            }
